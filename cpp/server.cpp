...
void handleClient(int clientSocket, string keyHex, string keyBin) {
    cout << "Thread [" << this_thread::get_id() << "] created for client " << clientSocket << endl;
    if (send(clientSocket, keyHex.c_str(), keyHex.length(), 0) < 0) {
    ...
    while(true) {
        ...
        size_t spaceFlag = pt.find(' ');

        if(spaceFlag != string::npos){
            command = pt.substr(0, spaceFlag);
            content = pt.substr(spaceFlag+1);
        }else{
            command = pt;
        }
        ...
        if(command =="encrypt"){
            if(content.empty()){
                result = "Error: No data provided. Usage: encrypt <msg>";
            }else{
                cout << "Thread [" << this_thread::get_id() << "] Received ENCRYPT command. Data: '" << content << "'" << endl;
                generateKeys(keyBin);
                string binary_content = ASCIItoBin(content);
                string padded_content = addPadding(binary_content);

                string full_ct = "";
                int total_blocks = padded_content.length() / 64;

                for (int i = 0; i < total_blocks; i++) {
                    string block_to_encrypt = padded_content.substr(i * 64, 64); 
                    string block_ct = DES(block_to_encrypt); 
                    full_ct += block_ct;
                }
                result = convertBinToHex(full_ct);
                cout << "Thread [" << this_thread::get_id() << "] Sending Encrypted (Hex): " << result << endl;
            }

        }else if(command == "decrypt"){
            if(content.empty()){ 
                result = "Error: No data provided. Usage: decrypt <hex>";
            } else { 
                cout << "Thread [" << this_thread::get_id() << "] Received DECRYPT command. Data (Hex): '" << content << "'" << endl;

            
                bool isHex = true;
                for(char c : content) { if (!isxdigit(c)) { isHex = false; break; } }

                if (!isHex) {
                    result = "Error: Invalid hexadecimal input for decryption.";
                } else {
                    string ct_bin = convertHexToBin(content); 
                    if (ct_bin.length() % 64 != 0) {
                        result = "Error: Invalid ciphertext length after hex conversion.";
                    } else {
                        string decrypted = decrypt(ct_bin, keyBin); 
                        result = decrypted;
                        cout << "Thread [" << this_thread::get_id() << "] Sending Decrypted (ASCII): " << result << endl;
                    }
                }
            } 
        }else {
            result = "Error: Invalid command. Use 'encrypt <data>' or 'decrypt <hex_data>'.";
            cout << "Thread [" << this_thread::get_id() << "] Invalid command received: '" << command << "'" << endl;
        }
...
int main()
{
    ...
    generateKeys(key);
    string keyhex = convertBinToHex(key);


    int serverSocket = socket(AF_INET, SOCK_STREAM, 0);
    if(serverSocket < 0){
        perror("Socket inisialization failed");
        return 1;
    }
    ...
    cout << "Server listening on 3000" << endl;
    ...
    while(true){
        ...
        thread workerThread(handleClient, clientSocket, keyhex, key);
        workerThread.detach();
    }
    close(serverSocket);
    return 0;
